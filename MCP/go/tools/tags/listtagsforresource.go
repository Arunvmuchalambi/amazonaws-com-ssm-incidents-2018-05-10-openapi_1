...
func CreateListtagsforresourceTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_tags_resourceArn",
		mcp.WithDescription("Lists the tags that are attached to the specified response plan."),
		mcp.WithString("resourceArn", mcp.Required(), mcp.Description("The Amazon Resource Name (ARN) of the response plan.")),
	)
