...
func CreateUntagresourceTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_tags_resourceArn#tagKeys",
		...
		mcp.WithString("resourceArn", mcp.Required(), mcp.Description("The Amazon Resource Name (ARN) of the response plan you're removing a tag from.")),
		mcp.WithArray("tagKeys", mcp.Required(), mcp.Description("The name of the tag to remove from the response plan.")),
	)
