...
func CreateTagresourceTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_tags_resourceArn",
		mcp.WithDescription("Adds a tag to a response plan."),
		mcp.WithString("resourceArn", mcp.Required(), mcp.Description("The Amazon Resource Name (ARN) of the response plan you're adding the tags to.")),
		mcp.WithObject("tags", mcp.Required(), mcp.Description("Input parameter: A list of tags to add to the response plan.")),
	)
